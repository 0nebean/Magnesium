<template>
  <div>
    <a @click="showDataList()" type="primary">添加IP访问白名单</a>
    ...
    >
      <Button @click="showAddFrom = true" type="info">新增</Button>
      <Divider />
    ...
    >
      ...
      >
        <FormItem label="ip地址" prop="ipAddress">
          <i-input
            placeholder="请输入应用名称"
            v-model="addDataParam.ipAddress"
          ></i-input>
        </FormItem>
        <FormItem>
          <Button type="primary" @click="handleSubmit('addDataParam')"
            >提交</Button
          >
        </FormItem>
      </Form>
  ...
  methods: {
    ...
    deleteData(id, index) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.utils.netUtil.post(
            this.API_PTAH.ipWhtieListDelete,
            { id: id },
            () => {
