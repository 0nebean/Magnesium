...
<template>
  <div>
      ...
      <i-col span="12" offset="6">
        ...
        >
          <FormItem label="服务名称" prop="serverName">
            <i-input
              v-model="serverInfoFrom.serverName"
              placeholder="请输入服务名称"
            ></i-input>
          </FormItem>

          <FormItem label="服务节点" prop="upsteamNodeName">
            <Select
              v-model="serverInfoFrom.upsteamNodeName"
              filterable
              clearable
              remote
              :label-in-value="true"
            ...
            >
              <Option
                v-for="(option, index) in upSteamNodeInfo"
                :value="option.upsteamName"
                :key="index"
                >{{ option.upsteamName }} -
                {{
                  option.deployType == '0' ? '物理部署' : 'kubernetes部署'
                }}</Option
              >
            </Select>
          ...
          >
            <Select
              v-model="serverInfoFrom.selectedVersion"
              @on-change="updateSelectedVersion()"
              placeholder="选择暴露的版本"
            >
              <Option
                v-for="item in selectedVersionEunmArr"
                :value="item.version"
          ...
          <FormItem label="是否需要域名" prop="isFront">
            <i-switch
              v-model="serverInfoFrom.isFront"
          ...
          >
            <i-input
              v-model="serverInfoFrom.listenPort"
              placeholder="请输入监听端口号"
            ></i-input>
          </FormItem>

          <FormItem label="域名" v-if="showOnNeedHost()" prop="serverHost">
            <i-input
              v-model="serverInfoFrom.serverHost"
              placeholder="请输域名"
            ></i-input>
          </FormItem>

          <FormItem label="是否开启ssl" v-if="showOnNeedHost()" prop="isSsl">
            <i-switch
              v-model="serverInfoFrom.isSsl"
          ...
          >
            <i-input
              v-model="serverInfoFrom.sslListenPort"
              placeholder="请输入ssl监听端口号"
            ></i-input>
          </FormItem>
          ...
          >
            <i-input
              v-model="serverInfoFrom.sslCrtKeyPath"
              placeholder="请输入证书key路径"
            ></i-input>
          </FormItem>

          <FormItem label="证书路径" v-if="showOnSslOn()" prop="sslCrtPath">
            <i-input
              v-model="serverInfoFrom.sslCrtPath"
              placeholder="请输入证书路径"
            ></i-input>
          </FormItem>
          ...
          <FormItem label="创建时间" prop="createTime">
            <i-input
              v-model="serverInfoFrom.createTime"
              disabled
              placeholder="请输入创建时间"
            ></i-input>
          </FormItem>
          ...
          <FormItem label="	修改时间" prop="updateTime">
            <i-input
              v-model="serverInfoFrom.updateTime"
              disabled
              placeholder="请输入修改时间"
            ></i-input>
          </FormItem>

          <FormItem label="	操作人" prop="operatorName">
            <i-input
              v-model="serverInfoFrom.operatorName"
              disabled
              placeholder="请输入操作人"
            ></i-input>
          </FormItem>
          ...
          <FormItem>
            <Button type="primary" @click="handleSubmit('serverInfoFrom')"
              >提交</Button
            >
          </FormItem>
        </Form>
    ...
    <Spin size="large" fix v-if="globalScreenLoding" />
  </div>
</template>
  ...
  methods: {
      ...
      if (query !== '') {
        ...
        this.utils.netUtil.post(
          ...
          response => {
            ...
            this.upSteamNodeInfo = response.data.data
          }
        )
      } else {
    ...
    loadData() {
      this.utils.netUtil.post(
        ...
        this.serverInfoFrom,
        response => {
          this.serverInfoFrom = response.data.data
    ...
    findVersionListByNodeName() {
      this.utils.netUtil.post(
        ...
        this.serverInfoFrom,
        res => {
          this.selectedVersionEunmArr = res.data.data
        }
      )
    },
