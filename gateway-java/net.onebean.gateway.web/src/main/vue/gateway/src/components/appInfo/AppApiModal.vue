    ...
    <Row>
      <i-col span="20" offset="2">
        <Transfer
          ...
          @on-change="handleBindingApi"
        ></Transfer>
      </i-col>
    </Row>
    <Spin size="large" fix v-if="loadingModalData"></Spin>
  </Modal>
</template>
  ...
  methods: {
    clearSelectServer() {
      ...
      this.utils.netUtil.post(
        ...
        response => {
          this.allApi = response.data.data.allApi
          this.bindKeys = response.data.data.bindKeys
        }
      )
    },
    closeModal: function() {
        ...
        this.utils.netUtil.post(
          ...
          response => {
            this.allApi = this.pushToHeavyAllApi(
              this.allApi,
              response.data.data.allApi
            )
            this.bindKeys = this.pushToHeavyBindKeys(
              this.bindKeys,
              response.data.data.bindKeys
            )
          }
        )
      }
      ...
      if (query !== '') {
        ...
        this.utils.netUtil.post(
          ...
          response => {
            ...
            this.serverOptions = response.data.data
            this.loadingModalData = false
          }
        )
      } else {
